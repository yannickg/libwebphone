		...
		function loadSIPjs() {
			kazooLoadScript(kazoo.config.paths.SIPjs, function(globalVar) {
				...
				kazoo.version = 'sipjs';
                                ...
                                kazoo.videoLocal = document.createElement('video');
                                kazoo.videoLocal.id = 'localVideo';
                                kazoo.videoLocal.muted = 'muted';
                                //kazoo.videoLocal.style.display = 'none';
                                kazoo.videoLocal.style.width = '10%';
                                kazoo.videoLocal.style.height = 'auto';
                                document.getElementsByTagName('body')[0].appendChild(kazoo.videoLocal);

				kazoo.videoRemote = document.createElement('video');
				kazoo.videoRemote.id = 'remoteVideo';
				//kazoo.videoRemote.style.display = 'none';
				kazoo.videoRemote.style.width = '100%';
				kazoo.videoRemote.style.height = 'auto';
	...
	kazoo.muteMicrophone = function(mute, success, error) {
		if(kazoo.version === 'rtmp') {
			...
			if(kazoo.sipjs.hasOwnProperty('session')) {
				if(mute) {
					kazoo.sipjs.session.mute({audio: true, video: false});
				}
				else {
					kazoo.sipjs.session.unmute({audio: true, video: false});
				}
